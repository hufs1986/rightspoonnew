...
`.trim();
    ...
    const fetchVideoTranscript = async () => {
        ...
        try {
            ...
            setVideoMessage(
                `자막을 재료에 추가했습니다${data.isAutoGenerated ? "(자동 생성)" : ""}. 기획안과 섞어서 그대로 두면 됩니다.`
            );
        } catch (err) {
    ...
    const generateColumn = async () => {
        if (!sourceText.trim() && !youtubeUrl.trim()) {
            alert("기획안·원기사를 붙여넣거나 유튜브 링크를 입력해 주세요.");
            return;
        }
        const hasDraft = title.trim() || content.trim();
                        ...
                        <div className={styles.videoDesk}>
                            <div className={styles.sourceDeskHeader}>
                                <div>
                                    ...
                                    <p>영상 기획안이나 참고한 원기사를 통째로 붙여넣으면 내 문체 그대로 칼럼을 씁니다. 생성 후 검토하고 발행하거나, 바로 발행할 수 있습니다.</p>
                                </div>
                            </div>
                            <div className={styles.formGroup}>
                                <label className={styles.label}>유튜브 영상 링크 (칼럼 아래 영상 재생용)</label>
                                <input
                                    type="url"
                            ...
                            <div className={styles.formGroup}>
                                <div className={styles.transcriptHeader}>
                                    <label className={styles.label}>
                                        기획안 · 원기사 · 메모{videoTranscriptTitle ? ` · ${videoTranscriptTitle}` : ""}
                                    </label>
                                    <button
                                        type="button"
                                        className={styles.templateBtn}
                                        onClick={fetchVideoTranscript}
                                        disabled={isFetchingTranscript || isGeneratingColumn || !youtubeUrl.trim()}
                                        title={youtubeUrl.trim() ? "영상 자막을 재료에 추가합니다" : "유튜브 링크를 먼저 입력하세요"}
                                    >
                                        {isFetchingTranscript ? "가져오는 중..." : "자막 가져와서 추가"}
                                    </button>
                                </div>
                                <textarea
                                    ...
                                    placeholder="영상을 만들 때 쓴 기획안, 하고 싶은 말, 반박할 프레임, 참고한 원기사 전문 등을 한꺼번에 붙여넣으세요. 섞여 있어도 됩니다 — AI가 논점을 뽑아내고 원기사는 근거로만 씁니다."
                                />
                            </div>
