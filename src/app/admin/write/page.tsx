...
export default function AdminWritePage() {
    ...
    const [videoTranscriptTitle, setVideoTranscriptTitle] = useState("");
    ...
    const [publishAfterGenerate, setPublishAfterGenerate] = useState(false);
...
`.trim();
    ...
    const fetchVideoTranscript = async () => {
        if (!youtubeUrl.trim()) {
            alert("유튜브 URL을 먼저 입력해 주세요.");
            return;
        }
        setIsFetchingTranscript(true);
        setVideoMessage("영상 자막을 불러오는 중입니다.");
        try {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data?.error || "자막을 가져오지 못했습니다.");
            setVideoTranscript(data.transcript);
            setVideoTranscriptTitle(data.title || "");
            setVideoMessage(
                `자막을 불러왔습니다${data.isAutoGenerated ? "(자동 생성)" : ""}. 오타나 잘못 인식된 부분을 고친 뒤 생성하세요.`
            );
        } catch (err) {
            console.error("Transcript fetch failed:", err);
            setVideoMessage(err instanceof Error ? err.message : "자막을 가져오지 못했습니다.");
        } finally {
    ...
    const generateColumn = async () => {
        if (!videoTranscript.trim() && !videoNotes.trim() && !youtubeUrl.trim()) {
            alert("유튜브 URL, 대본, 또는 기획 내용을 입력해 주세요.");
            return;
        }
        const hasDraft = title.trim() || content.trim();
        if (!publishAfterGenerate && hasDraft && !confirm("현재 작성 중인 제목/본문을 생성된 칼럼으로 교체할까요?")) return;
        if (publishAfterGenerate && !confirm("검토 없이 바로 발행됩니다. 진행할까요?")) return;

        setIsGeneratingColumn(true);
        setVideoMessage("드럼통119 문체로 칼럼을 작성하는 중입니다.");
        try {
            const res = await fetch("/api/ai/column", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    ...
                    videoTitle: videoTranscriptTitle,
                    memo: videoMemo.trim(),
                    publish: publishAfterGenerate,
                }),
            });
            const data = await res.json();
            if (!res.ok) throw new Error(data?.error || "칼럼 생성에 실패했습니다.");

            if (data.published) {
                alert("칼럼이 발행되었습니다.");
                window.location.href = data.published.path;
                return;
            }
            ...
            setVideoMessage(
                data?.message ||
                (data?.mode === "ai"
                    ? "칼럼을 생성했습니다. 본문을 확인한 뒤 발행하세요."
                    : "원고 초안으로 채웠습니다.")
            );
        } catch (err) {
            console.error("Column generation failed:", err);
            setVideoMessage(err instanceof Error ? err.message : "칼럼 생성에 실패했습니다.");
        } finally {
                    ...
                    <form className={styles.adminForm} onSubmit={handleSubmit}>
                        <div className={styles.videoDesk}>
                            <div className={styles.sourceDeskHeader}>
                                <div>
                                    <strong>영상 → 칼럼 작업대</strong>
                                    <p>영상을 만들 때의 기획 내용이나 대본을 넣으면 내 문체 그대로 칼럼을 씁니다. 자막은 보조 자료입니다. 생성 후 검토하고 발행하거나, 바로 발행할 수 있습니다.</p>
                                </div>
                            </div>
                            <div className={styles.sourceGrid}>
                                <div className={styles.formGroup}>
                                    <label className={styles.label}>유튜브 영상 URL</label>
                                    <input
                                        type="url"
                                ...
                                <div className={styles.formGroup}>
                                    <label className={styles.label}>추가 지시 (선택)</label>
                                    <input
                                        type="text"
                                        className={styles.input}
                                        value={videoMemo}
                                        onChange={(e) => setVideoMemo(e.target.value)}
                                        placeholder="예: 마지막에 대출규제 이야기로 연결"
                                    />
                                </div>
                            </div>
                            <div className={styles.formGroup}>
                                <label className={styles.label}>영상 기획 내용 / 핵심 주장</label>
                                <textarea
                                    className={styles.transcriptBox}
                                    value={videoNotes}
                                    onChange={(e) => setVideoNotes(e.target.value)}
                                    placeholder="영상을 만들 때의 기획 의도, 하고 싶은 말, 반박할 프레임, 넣고 싶은 비유 등을 거칠게 적어도 됩니다. 이게 칼럼의 뼈대가 됩니다."
                                />
                            </div>
                            <div className={styles.formGroup}>
                                <div className={styles.transcriptHeader}>
                                    <label className={styles.label}>
                                        영상 대본·자막 (선택) {videoTranscriptTitle ? `· ${videoTranscriptTitle}` : ""}
                                    </label>
                                    <button
                                        type="button"
                                        className={styles.templateBtn}
                                        onClick={fetchVideoTranscript}
                                        disabled={isFetchingTranscript || isGeneratingColumn}
                                    >
                                        {isFetchingTranscript ? "자막 불러오는 중..." : "자막 불러오기"}
                                    </button>
                                </div>
                                <textarea
                                    className={styles.transcriptBox}
                                    ...
                                    placeholder="자막 불러오기를 누르거나, 영상 대본을 직접 붙여넣으세요."
                                />
                            </div>
                            <div className={styles.videoActions}>
                                <label className={styles.publishCheck}>
                                    <input
                                        ...
                                        onChange={(e) => setPublishAfterGenerate(e.target.checked)}
                                    />
                                    생성 후 바로 발행
                                </label>
                                <button
                                    ...
                                    disabled={isGeneratingColumn || isFetchingTranscript}
                                >
                                    {isGeneratingColumn ? "칼럼 작성 중..." : "내 문체로 칼럼 생성"}
                                </button>
                            </div>
                            {videoMessage && <div className={styles.draftMessage}>{videoMessage}</div>}
                        </div>
